...
namespace AFUT.Tests.UnitTests.Reports.TypesOfReports
{
    ...
    public class AccreditationTests : IClassFixture<AppConfig>
    {
        private readonly AppConfig _config;
        private readonly IPookieDriverFactory _driverFactory;
        private readonly ITestOutputHelper _output;

        public AccreditationTests(AppConfig config, ITestOutputHelper output)
        {
        ...
        [Fact]
        [TestPriority(1)]
        public void VerifyAccreditationReportCriteriaAndFilters()
        {
            using var driver = _driverFactory.CreateDriver();

            // Use the helper method to test the complete Accreditation filter functionality
            ReportFilterHelper.TestReportFilterCategoryComplete(driver, _config, _output, "Accreditation");
        }

        [Fact]
        [TestPriority(2)]
        public void VerifyAccreditationReportPdfContentMatchesSelectedCriteria()
        {
            ...
            try
            {
                ...
                // Step 1: Navigate to Reports > Accreditation
                _output.WriteLine("[INFO] Navigating to Accreditation reports...");
                var homePage = CommonTestHelper.NavigateToReportsHomePage(driver, _config, _output);
                ReportFilterHelper.SelectReportFilter(driver, _output, "Accreditation");
                
                // Change page size to "All" to test all reports
                ReportFilterHelper.ChangePageSizeToAll(driver, _output);
                
                // Get count of all Accreditation reports
                var reportCount = ReportFilterHelper.GetReportCount(driver, _output);
                _output.WriteLine($"\n{'=',-60}");
                _output.WriteLine($"[INFO] Testing PDF generation for all {reportCount} Accreditation reports");
                _output.WriteLine($"{'=',-60}");

                var random = new Random();

                // Test each Accreditation report
                for (int i = 0; i < reportCount; i++)
                {
                    ...
                    var reportName = ReportFilterHelper.ClickReportAtIndex(driver, _output, "Accreditation", i);
                ...
                _output.WriteLine($"\n{'=',-60}");
                _output.WriteLine($"[PASS] All {reportCount} Accreditation reports PDF validation completed successfully");
                _output.WriteLine($"{'=',-60}");
            }
            finally
            {
