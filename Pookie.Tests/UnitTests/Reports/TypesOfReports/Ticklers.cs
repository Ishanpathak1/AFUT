...
namespace AFUT.Tests.UnitTests.Reports.TypesOfReports
{
    ...
    public class TicklersTests : IClassFixture<AppConfig>
    {
        private readonly AppConfig _config;
        private readonly IPookieDriverFactory _driverFactory;
        private readonly ITestOutputHelper _output;

        public TicklersTests(AppConfig config, ITestOutputHelper output)
        {
        ...
        [Fact]
        [TestPriority(1)]
        public void VerifyTicklersReportCriteriaAndFilters()
        {
            using var driver = _driverFactory.CreateDriver();

            // Use the helper method to test the complete Ticklers filter functionality
            ReportFilterHelper.TestReportFilterCategoryComplete(driver, _config, _output, "Ticklers");
        }

        [Fact]
        [TestPriority(2)]
        public void VerifyTicklersReportPdfContentMatchesSelectedCriteria()
        {
            ...
            try
            {
                ...
                // Navigate to Reports > Ticklers
                _output.WriteLine("[INFO] Navigating to Ticklers reports...");
                var homePage = CommonTestHelper.NavigateToReportsHomePage(driver, _config, _output);
                ReportFilterHelper.SelectReportFilter(driver, _output, "Ticklers");
                
                // Change page size to "All" to test all reports
                ReportFilterHelper.ChangePageSizeToAll(driver, _output);
                
                // Get count of all Ticklers reports
                var reportCount = ReportFilterHelper.GetReportCount(driver, _output);
                _output.WriteLine($"\n{'=',-60}");
                _output.WriteLine($"[INFO] Testing PDF generation for all {reportCount} Ticklers reports");
                _output.WriteLine($"{'=',-60}");

                var random = new Random();

                // Test each Ticklers report
                for (int i = 0; i < reportCount; i++)
                {
                    ...
                    var reportName = ReportFilterHelper.ClickReportAtIndex(driver, _output, "Ticklers", i);

                    // Randomize criteria (like a real user) and track what was selected
                    var selectedCriteria = ReportFilterHelper.RandomizeTicklersCriteria(driver, _output, random);
                    ...
                    // Validate PDF content matches what we selected
                ...
                _output.WriteLine($"\n{'=',-60}");
                _output.WriteLine($"[PASS] All {reportCount} Ticklers reports PDF validation completed successfully");
                _output.WriteLine($"{'=',-60}");
            }
            finally
            {
